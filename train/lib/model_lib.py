...
class AzureMLHook(tf.train.SessionRunHook):
  def __init__(self, values):
    self._run = Run.get_submitted_run()
    self._values = values

  def begin(self):
    self._current = {tag: _as_graph_element(tensor)
                     for (tag, tensor) in self._values.items()}
  
  def before_run(self, run_context):
    return tf.train.SessionRunArgs(self._current)
  
  def after_run(self, run_context, run_values):
    self._run.log('loss', run_values.results['loss'])
...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if use_tpu and mode != tf.estimator.ModeKeys.EVAL:
      ...
      hook = AzureMLHook({'loss' : total_loss})
